...
df <- df %>% filter(!is.na(RIDAGEYR) & sexFlag == 0) 
...
plot_sub <- df %>% 
  ...
  rename(DNAmTL_AA = HorvathTelo_AA) # renaming DNAmTL to the more common name
...
```{r}
load("NHANES_Exposures_Clean.RData")
dat <- df %>% filter(!is.na(RIDAGEYR) & sexFlag == 0) 
...
# subset to true reported age and sex-mismatch removed sample
nhanesDesign <- subset(nhanesDesign, !is.na(RIDAGEYR) & sexFlag == 0)
...
a <- spl$GrimAge_AA %>% 
  ...
  ylab(bquote(bold(-log[10](p~value)))) + 
  theme(legend.position = "bottom", legend.title = element_blank()) + 
  scale_x_continuous(limits = c(-1.4, 2.33)) + 
  ...
  geom_text(data=subset(spl$GrimAge_AA, exposuretag != ""),
            aes(x = ifelse(exposuretag == "LBXBCD" | exposuretag == "URDUCD", 
                ...
                y = ifelse(exposuretag == "LBX118LA", -0.08 + (-log10(pval)), 
                                         0.18 + (-log10(pval))),  
                label=Variable.Description, color = Dataset), 
            hjust = 0, 
            show.legend = FALSE) + 
...
b <- spl$GrimAge2_AA %>% 
  ...
  ggtitle("GrimAge2: Cell-Adjusted Models") + 
  geom_text(data=subset(spl$GrimAge_AA, exposuretag != ""),
            aes(x = ifelse(exposuretag == "LBXBCD" | exposuretag == "URDUCD", Estimate - 0.74, 
                           Estimate + 0.1), 
...
a <- spl$GrimAge2_AA %>% 
  ...
  geom_text(data=subset(spl$GrimAge2_AA, exposuretag != ""),
            aes(x = Estimate, y = -log10(pval),  
                label=Variable.Description, color = Dataset), 
            hjust = 0, nudge_x = -0.2*(max(spl$GrimAge2_AA$Estimate) - min(spl$GrimAge2_AA$Estimate)), 
            nudge_y = -0.1, 
            show.legend = FALSE) + 
  theme(plot.title = element_text(hjust = 0.5, face="bold", size = 12), 
        axis.text = element_text(face="bold", size = 12), 
...
c <- spl$DunedinPoAm %>% 
  ...
  geom_text(data=subset(spl$DunedinPoAm, exposuretag != ""),
            aes(x = Estimate, y = -log10(pval),  
                label=Variable.Description, color = Dataset), 
            hjust = 0, nudge_x = -0.1*(max(spl$DunedinPoAm$Estimate) - min(spl$DunedinPoAm$Estimate)), 
            nudge_y = 0.2*(max(spl$DunedinPoAm$Estimate) - min(spl$DunedinPoAm$Estimate)), 
            show.legend = FALSE) + 
  theme(plot.title = element_text(hjust = 0.5, face="bold", size = 12), 
        axis.text = element_text(face="bold", size = 12), 
        axis.title = element_text(face="bold", size = 12), 
...
# Sensitivity - Check Associations with Diet
```{r}
library(tidyverse)
library(tableone)
library(Metrics)
...
# specific meat and dairy 
diet_1999 <- haven::read_xpt("Exploratory/DBQ.XPT")    # specific questions only present in 1999 cycle
diet_1999 <- diet_1999 %>% filter(SEQN %in% df$SEQN) %>% select(SEQN, DBD270A, DBD270B)
table(subset(diet_1999, DBD270A < 20)$DBD270A)
table(subset(diet_1999, DBD270B < 20)$DBD270B)
df <- left_join(df, diet_1999, by = "SEQN")
df <- df %>% 
  mutate(protein = ifelse(DBD270A == 0, 0, 
                   ifelse(DBD270A >= 1 & DBD270A < 3, 1, 
                   ifelse(DBD270A >= 3, 2, NA))), 
         dairy = ifelse(DBD270B == 0, 0, 
                   ifelse(DBD270B >= 1 & DBD270B < 3, 1, 
                   ifelse(DBD270B >= 3, 2, NA))))

ggplot(df, aes(x = factor(protein), y = LBX118LA)) + 
  geom_boxplot()
ggplot(df, aes(x = factor(dairy), y = LBX118LA)) + 
  geom_boxplot()
fit <- glm(LBX118LA ~ factor(protein), data = df); summary(fit)
fit <- glm(LBX118LA ~ factor(dairy), data = df); summary(fit)




# dairy intake (DBD195, DBD196)
d1 <- haven::read_xpt("Exploratory/DBQ.XPT") 
d2 <- haven::read_xpt("Exploratory/DBQ_B.XPT") 
d1 <- d1 %>% select(SEQN, DBD195)
d2 <- d2 %>% rename(DBD195 = DBD196) %>% select(SEQN, DBD195)
d <- rbind(d1, d2)
df <- left_join(df, d, by = "SEQN")
table(df$DBD195)
fit <- glm(log2(LBX118LA) ~ factor(DBD195), data = subset(df, DBD195 < 4)); summary(fit)
fit <- glm(log2(LBXD05LA) ~ factor(DBD195), data = subset(df, DBD195 < 4)); summary(fit)


# overall protein intake (DBD270A: only reported in 1999)
d1 <- haven::read_xpt("Exploratory/DBQ.XPT") 
d1 <- d1 %>% select(SEQN, DBD270A) %>% mutate(protein_cat = ifelse(DBD270A == 0, 0, 
                   ifelse(DBD270A >= 1 & DBD270A < 3, 1, 
                   ifelse(DBD270A >= 3, 2, NA))))
df <- left_join(df, d1, by = "SEQN")
fit <- glm(log2(LBX118LA) ~ factor(protein_cat), data = df); summary(fit)
fit <- glm(log2(LBXD05LA) ~ factor(protein_cat), data = df); summary(fit)


# fish intake (DRQ360, DRD360)
d1 <- haven::read_xpt("Exploratory/DRXTOT.XPT") 
d2 <- haven::read_xpt("Exploratory/DRXTOT_B.XPT") 
d1 <- d1 %>% select(SEQN, DRQ360)
d2 <- d2 %>% rename(DRQ360 = DRD360) %>% select(SEQN, DRQ360)
d <- rbind(d1, d2)
df <- left_join(df, d, by = "SEQN")
fit <- glm(log2(LBX118LA) ~ factor(DRQ360), data = subset(df, DBD195 < 3)); summary(fit)
fit <- glm(log2(LBXD05LA) ~ factor(DRQ360), data = subset(df, DBD195 < 3)); summary(fit)

```

Include fish consumption as a covariate
```{r}
library(haven)
library(tidyverse)
library(Metrics)
...
# create new variable name for easier model specs
nhanesDesign[["variables"]]$cur_measure <- unlist(nhanesDesign[["variables"]]["LBX118LA"]) 
nhanesDesign[["variables"]]$cur_measure <- as.numeric(nhanesDesign[["variables"]]$cur_measure)
nhanesDesign[["variables"]]$cur_measure_scale <- log2(nhanesDesign[["variables"]]$cur_measure)
nhanesDesign[["variables"]]$cur_measure_scale <- as.vector(scale(
  nhanesDesign[["variables"]]$cur_measure_scale, center = TRUE, scale = TRUE))

weighted_model <- svyglm(GrimAge_AA ~ cur_measure_scale + 
                       RIDAGEYR + I(RIDAGEYR^2) + RIAGENDR + EDUCATION + OCCUPATION + 
                         RIDRETH1 + BMXBMI + INDFMPIR + LBXCOT + DBD195, 
