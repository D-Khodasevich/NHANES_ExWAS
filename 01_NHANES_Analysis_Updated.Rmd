...
## Revision Models
Cadmium exposure, stratified by smoking status
```{r}

nhanesDesign[["variables"]]$cur_measure <- unlist(nhanesDesign[["variables"]]["LBXBCD"]) 
nhanesDesign[["variables"]]$cur_measure <- as.numeric(nhanesDesign[["variables"]]$cur_measure)
nhanesDesign[["variables"]]$cur_measure_scale <- log2(nhanesDesign[["variables"]]$cur_measure)
nhanesDesign[["variables"]]$cur_measure_scale <- as.vector(scale(nhanesDesign[["variables"]]$cur_measure_scale, 
                                                   center = TRUE, scale = TRUE))
...
# grimage
weighted_model <- svyglm(GrimAge_AA ~ cur_measure_scale + RIDAGEYR + I(RIDAGEYR^2) + 
                           RIAGENDR + EDUCATION + OCCUPATION + RIDRETH1 + BMXBMI + INDFMPIR, 
                         design = subset(nhanesDesign, SMOKE_3cat == "Never"), family = gaussian())
summary(weighted_model)

weighted_model <- svyglm(GrimAge_AA ~ cur_measure_scale + RIDAGEYR + I(RIDAGEYR^2) + 
                           RIAGENDR + EDUCATION + OCCUPATION + RIDRETH1 + BMXBMI + INDFMPIR, 
                         design = subset(nhanesDesign, SMOKE_3cat == "Former"), family = gaussian())
summary(weighted_model)

weighted_model <- svyglm(GrimAge_AA ~ cur_measure_scale + RIDAGEYR + I(RIDAGEYR^2) + 
                           RIAGENDR + EDUCATION + OCCUPATION + RIDRETH1 + BMXBMI + INDFMPIR, 
                         design = subset(nhanesDesign, SMOKE_3cat == "Current"), family = gaussian())
summary(weighted_model)

# grimage 2
weighted_model <- svyglm(GrimAge2_AA ~ cur_measure_scale + RIDAGEYR + I(RIDAGEYR^2) + 
                           RIAGENDR + EDUCATION + OCCUPATION + RIDRETH1 + BMXBMI + INDFMPIR, 
                         design = subset(nhanesDesign, SMOKE_3cat == "Never"), family = gaussian())
summary(weighted_model)
...
# DunedinPoAm
weighted_model <- svyglm(DunedinPoAm ~ cur_measure_scale + RIDAGEYR + I(RIDAGEYR^2) + 
                           RIAGENDR + EDUCATION + OCCUPATION + RIDRETH1 + BMXBMI + INDFMPIR, 
                         design = subset(nhanesDesign, SMOKE_3cat == "Never"), family = gaussian())
summary(weighted_model)
...
df$ageFlag = ifelse(is.na(df$RIDAGEYR), 1, 0)
...
# subset to true reported age and sex-mismatch removed sample
nhanesDesign <- subset(nhanesDesign, ageFlag == 0 & sexFlag == 0)
